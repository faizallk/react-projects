      ...
      <div className="w-[85%] h-screen bg-white flex flex-wrap overfow-x-hidden overflow-y-auto p-10 pt-[5%]">
        {filteredProduct && filteredProduct.map((p, i) => (
          <Link
            key={i}
            to={`/details/${p.id}`}
            className="m-auto shadow border rounded border-zinc-300 mb-5 card w-50 h-55 text-ellipsis overflow-hidden bg-white py-5 px-2 text-center hover:shadow-zinc-400 duration-200"
          >
            <div
